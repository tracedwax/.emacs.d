# Agenda Priority Tiers

## Purpose

The manual priority-tier layer on top of org-agenda: a mutually-exclusive ladder of tiers — On Fire → Bells Ringing → On Deck → Recently Considered → Paused — set by single agenda keys, with tiered tasks grouped tier-first (and per-tier effort totals) in the agenda views. Distinct from, and orthogonal to, the score-based "big rock" bucketing.
