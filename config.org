...
#+BEGIN_SRC emacs-lisp
  ...
  (defun tdw/get-sanity-effort-totals ()
    "Return a list (TOTAL EFFORT-A EFFORT-B EFFORT-C EFFORT-D) formatted as H:MM."
    (let ((mins-a 0)
          (mins-b 0)
          (mins-c 0)
          (mins-d 0))
                  ...
                  (let ((priority (org-entry-get (point) "PRIORITY"))
                    ...
                    (when (member todo '("NEXT" "TODO"))
                      (let ((effort (tdw/get-subtree-effort-minutes)))
                        (cond ((string= priority "A") (setq mins-a (+ mins-a effort)))
                              ((string= priority "B") (setq mins-b (+ mins-b effort)))
                              ((string= priority "C") (setq mins-c (+ mins-c effort)))
                              ((string= priority "D") (setq mins-d (+ mins-d effort))))))))
                nil nil))))))
      (let ((fmt (lambda (m) (format "%d:%02d" (/ m 60) (% m 60)))))
        (list (funcall fmt (+ mins-a mins-b mins-c mins-d))
              (funcall fmt mins-a)
              (funcall fmt mins-b)
              (funcall fmt mins-c)
              (funcall fmt mins-d)))))
  ...
  (defun tdw/update-sanity-view-headers ()
    "Update headers in the current agenda buffer with fresh effort totals."
    (let ((inhibit-read-only t))
      (save-excursion
        (goto-char (point-min))
        ;; Detect if we are in Sanity/Decision View by looking for our header markers
        (when (or (re-search-forward "Sanity View" nil t)
          ...
          (pcase-let ((`(,total ,effort-a ,effort-b ,effort-c ,effort-d) (tdw/get-sanity-effort-totals)))
            ...
            (while (re-search-forward "\\(Hot Tickets\\( (Priority A - [^)]*)\\)?\\)" nil t)
              (replace-match (format "Hot Tickets (Priority A - %s)" effort-a) t t nil 1))
            (goto-char (point-min))
            (while (re-search-forward "\\(For Today\\( (Priority B - [^)]*)\\)?\\)" nil t)
              (replace-match (format "For Today (Priority B - %s)" effort-b) t t nil 1))
            (goto-char (point-min))
            (while (re-search-forward "\\(Priority C\\( ([^)]*)\\)?\\)" nil t)
              (replace-match (format "Priority C (%s)" effort-c) t t nil 1))
            (goto-char (point-min))
            (while (re-search-forward "\\(Priority D\\( ([^)]*)\\)?\\)" nil t)
              (replace-match (format "Priority D (%s)" effort-d) t t nil 1)))))))
  ...
  (defun tdw-agenda-effort-increment () (interactive) (tdw/agenda-adjust-effort 15))
  (defun tdw-agenda-effort-decrement () (interactive) (tdw/agenda-adjust-effort -15))
...
- a string - returned as-is (literal)"
  ...
  (defun tdw-sanity-view ()
    "A highly focused GTD view: Today's schedule/habits, Priority A ('hot tickets') and Priority B ('for today') only."
    (interactive)
    (pcase-let ((`(,total ,effort-a ,effort-b ,effort-c ,effort-d) (tdw/get-sanity-effort-totals)))
      (org-gtd-view-show
       `((name . ,(format "Sanity View (Total Effort: %s)" total))
         (prefix . (effort "—"))
         (blocks . (
                    ;; 0. Today's Schedule (Calendar + Habits) — identical to engage view
                    ((name . "Today's Schedule")
                     (block-type . calendar-day))

                    ;; 1. Overdue Delegated check-ins
                    ((name . "Overdue Delegated")
                     (type . delegated)
                     (when . past))

                    ;; 2. Delegated check-ins due today
