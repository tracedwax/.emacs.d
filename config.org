...
;    :custom
;    (claude-code-ide-cli-path "~/.asdf/shims/claude")
;    :config
...
#+begin_src elisp
  ...
  ;  :config
  ;  (setq gptel-model 'gemma2:9b-instruct-q6_K)
...
;    (mcp-hub-servers '()))
#+end_src
** combobulate (tree-sitter-based navigation)
#+begin_src elisp
...
;   :hook ((prog-mode . combobulate-mode))
   ...
   ;; code.
;   :load-path ("/home/stag/src/vendor/combobulate"))
#+end_src
** evil-mode
...
#+BEGIN_SRC emacs-lisp
   ...
   ;; (use-package org-ql :after org :straight t)
    ...
    '(org-gtd :url "https://github.com/Trevoke/org-gtd.el"
       ...
       (use-package org-gtd
         ...
         ;; :straight (org-gtd :fetcher github :repo "trevoke/org-gtd.el" :branch "org-gtd-4")
         :demand t
         :init
         (setq org-gtd-prop-refile "ORG_GTD_REFILE")
     ...
     (require 'org-gtd)
  (defun tdw/skip-unless-habit ()
    "Skip everything that is not a GTD Habit."
    (let ((gtd-val (org-entry-get (point) "ORG_GTD")))
      (if (and (equal gtd-val "Habit")
               (not (org-entry-is-done-p)))
          nil ;; It's a habit, keep it!
  ...
  (defun tdw-sanity-view ()
   ...
   (let ((org-agenda-custom-commands
          `(("z" "Sanity View"
             (
              ;; 1. Habits (The "Pretty" Graph)
              (agenda "" ((org-agenda-span 1)
                          (org-agenda-start-day nil)
                          (org-agenda-skip-function 'tdw/skip-unless-habit)
                          (org-agenda-overriding-header "Habits")))

              ;; 2. Must Do (Priority A)
              (tags-todo "ORG_GTD=\"Actions\"/NEXT"
                         ((org-agenda-overriding-header "Must Do (Priority A)")
                          (org-agenda-skip-function '(org-agenda-skip-entry-if 'notregexp "\\[#A\\]"))))

              ;; 3a. Waiting On (Overdue)
              (tags-todo "ORG_GTD=\"Delegated\"/WAIT"
                         ((org-agenda-overriding-header "Waiting On (Overdue)")
                          (org-agenda-skip-function '(org-agenda-skip-entry-if 'scheduled 'future))))

              ;; 3b. Waiting On (Today)
              (tags-todo "ORG_GTD=\"Delegated\"/WAIT"
                         ((org-agenda-overriding-header "Waiting On (Today)")
                          (org-agenda-skip-function '(org-agenda-skip-entry-if 'not-scheduled 'today))))

              ;; 4. Should Do (Priority B)
              (tags-todo "ORG_GTD=\"Actions\"/NEXT"
                         ((org-agenda-overriding-header "Should Do (Priority B)")
                          (org-agenda-skip-function '(org-agenda-skip-entry-if 'notregexp "\\[#B\\]"))))
             ))))
         ;; Adjust habit graph column to the right (approx 15 chars right of default ~45)
...
#+begin_src elisp
    ...
    (use-package cargo-transient :after rust-mode :pin melpa)
    (use-package cargo-mode :after rust-mode)
    (use-package rust-mode :after eglot
    ...
    (use-package ob-rust :after org :pin melpa)

    (use-package rust-auto-use :after rust-mode :pin melpa)
#+end_src
