...
#+BEGIN_SRC emacs-lisp
  ...
  (defun tdw/get-sanity-effort-totals ()
    "Return a list (TOTAL EFFORT-A EFFORT-B) formatted as H:MM."
    (let ((mins-a 0)
          (mins-b 0))
      ...
      (let ((fmt (lambda (m) (format "%d:%02d" (/ m 60) (% m 60)))))
        (list (funcall fmt (+ mins-a mins-b))
              (funcall fmt mins-a)
              (funcall fmt mins-b)))))
    ...
    (let ((inhibit-read-only t))
      (save-excursion
        ...
        (when (or (re-search-forward "Sanity View" nil t)
          ...
          (pcase-let ((`(,total ,effort-a ,effort-b) (tdw/get-sanity-effort-totals)))
            (goto-char (point-min))
...
- a string - returned as-is (literal)"
  ...
  (defun tdw-sanity-view ()
    "A focused GTD view: Priority A ('hot tickets') and Priority B ('for today')."
    (interactive)
    (pcase-let ((`(,total ,effort-a ,effort-b) (tdw/get-sanity-effort-totals)))
      (org-gtd-view-show
