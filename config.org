...
#+BEGIN_SRC emacs-lisp
  ...
  (defun tdw/get-sanity-effort-totals ()
    "Return a list (TOTAL EFFORT-A EFFORT-B EFFORT-C) formatted as H:MM."
    (let ((mins-a 0)
          (mins-b 0)
          (mins-c 0))
      ...
      (let ((fmt (lambda (m) (format "%d:%02d" (/ m 60) (% m 60)))))
        (list (funcall fmt (+ mins-a mins-b mins-c))
              (funcall fmt mins-a)
              (funcall fmt mins-b)
              (funcall fmt mins-c)))))
  ...
  (defun tdw/update-sanity-view-headers ()
    ...
    (let ((inhibit-read-only t))
      (save-excursion
        (goto-char (point-min))
        ;; Detect if we are in Sanity View by looking for our header markers
        (when (or (re-search-forward "Sanity View" nil t)
                  (re-search-forward "Hot Tickets" nil t)
                  (re-search-forward "For Today" nil t)
                  (re-search-forward "After today" nil t))
          (pcase-let ((`(,total ,effort-a ,effort-b ,effort-c) (tdw/get-sanity-effort-totals)))
            (goto-char (point-min))
            ...
            (goto-char (point-min))
            (while (re-search-forward "\\(After today - Priority C\\( ([^)]*)\\)?\\)" nil t)
              (replace-match (format "After today - Priority C (%s)" effort-c) t t nil 1)))))))
...
- a string - returned as-is (literal)"
  ...
  (defun tdw-sanity-view ()
    "A focused GTD view: Priority A ('hot tickets'), Priority B ('for today'), and Priority C ('after today')."
    (interactive)
    (pcase-let ((`(,total ,effort-a ,effort-b ,effort-c) (tdw/get-sanity-effort-totals)))
      (org-gtd-view-show
       `((name . ,(format "Sanity View (Total Effort: %s)" total))
         ...
         (blocks . (
                    ...
                    ;; 3. "C" Items
                    ((name . ,(format "After today - Priority C (%s)" effort-c))
                     (type . next-action)
                     (todo . ("NEXT" "TODO"))
                     (priority . C))
                    ))))))
