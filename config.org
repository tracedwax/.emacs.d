...
#+BEGIN_SRC emacs-lisp
  ...
  (defun tdw/calculate-total-effort (priorities)
    "Sum effort for tasks with PRIORITIES (list of strings like 'A', 'B')."
    (let ((total-minutes 0))
               ...
               (org-map-entries
                (lambda ()
                  (let ((priority (org-entry-get (point) "PRIORITY"))
                    ...
                    (when (and (member priority priorities)
                               (string= todo "NEXT"))
                      (setq total-minutes (+ total-minutes (tdw/get-subtree-effort-minutes))))))
                nil nil))))))
  ...
  (defun tdw/update-sanity-view-headers ()
    ...
    (when (string-match-p "Sanity View" (or org-agenda-overriding-header ""))
      (let* ((effort-a (tdw/calculate-total-effort '("A")))
             (effort-b (tdw/calculate-total-effort '("B")))
             (total-effort (tdw/calculate-total-effort '("A" "B")))
             (inhibit-read-only t))
        (save-excursion
          (goto-char (point-min))
          (while (re-search-forward "Sanity View (Total Effort: [^)]*)" nil t)
            (replace-match (format "Sanity View (Total Effort: %s)" total-effort)))
          (goto-char (point-min))
          (while (re-search-forward "Hot Tickets (Priority A - [^)]*)" nil t)
            (replace-match (format "Hot Tickets (Priority A - %s)" effort-a)))
          (goto-char (point-min))
          (while (re-search-forward "For Today (Priority B - [^)]*)" nil t)
            (replace-match (format "For Today (Priority B - %s)" effort-b)))))))
...
- a string - returned as-is (literal)"
  ...
  (defun tdw-sanity-view ()
    ...
    (let* ((effort-a (tdw/calculate-total-effort '("A")))
           ...
           (total-effort (tdw/calculate-total-effort '("A" "B"))))
      (org-gtd-view-show
       `((name . ,(format "Sanity View (Total Effort: %s)" total-effort))
         (prefix . (project effort "—"))
         (blocks . (
                    ;; 1. "A" Items
