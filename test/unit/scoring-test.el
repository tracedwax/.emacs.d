...
;; regress them:
;;
;;   - tdw/score-at-point (KEYSTONE): floor((2*urg + imp) / tier),
;;     where tier comes from EFFORT minutes (0-5=1, 6-15=2, 16-60=3, 60+=4).
;;   - tdw/entry-score-info: the positional list skip functions read,
...
(deftest scoring/score-vh-vh-effort-1h-is-5 ()
  ...
  (with-temp-buffer
    ...
    (goto-char (point-min))
    (assert-equal 5 (tdw/score-at-point))))
...
(deftest scoring/score-l-l-no-effort-is-0 ()
  ...
  (with-temp-buffer
    ...
    (goto-char (point-min))
    (assert-equal 0 (tdw/score-at-point))))
...
Asymmetry check: urgency is weighted 2x, so h/m beats m/h."
  (with-temp-buffer
    ...
    (goto-char (point-min))
    (assert-equal 7 (tdw/score-at-point))))
...
Boundary: 5 minutes is still tier 1."
  (with-temp-buffer
    ...
    (goto-char (point-min))
    (assert-equal 15 (tdw/score-at-point))))
...
(deftest scoring/score-wh-wh-no-effort-is-24 ()
  ...
  (with-temp-buffer
    ...
    (goto-char (point-min))
    (assert-equal 24 (tdw/score-at-point))))
...
Big-effort items get divided down hard."
  (with-temp-buffer
    ...
    (goto-char (point-min))
    (assert-equal 2 (tdw/score-at-point))))
...
;;;; ——— tdw/entry-score-info: positional list ———

(deftest scoring/score-effort-list-length-is-6 ()
  "The list returned by tdw/entry-score-info has exactly 6 elements:
(score effort-mins p0-p done-p gc-next-p flyby-p)."
  (with-temp-buffer
    (org-mode)
    (insert "* TODO Test task  :vh_urgency:vh_impact:\n")
    ...
    (org-set-property "EFFORT" "1:00")
    (goto-char (point-min))
    (assert-equal 6 (length (tdw/entry-score-info)))))
...
score 5, effort-mins 60, no p0/done/gc_next/flyby."
  (with-temp-buffer
    ...
    (org-set-property "EFFORT" "1:00")
    (goto-char (point-min))
    (assert-equal '(5 60 nil nil nil nil) (tdw/entry-score-info))))
...
score 0 and effort-mins nil (unestimated, distinct from 0)."
  (with-temp-buffer
    ...
    (insert "* TODO Test task  :l_urgency:l_impact:\n")
    (goto-char (point-min))
    (assert-equal '(0 nil nil nil nil nil) (tdw/entry-score-info))))
