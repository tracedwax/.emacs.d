...
;;
;;   - tdw/get-subtree-effort-minutes   (sums remaining/non-DONE descendant effort)
;;   - tdw/--effort-counted-p           (would an entry be counted?)
;;   - tdw/--effort-ancestor-counted-p  (is an ancestor already counted?)
;;
...
;; buffers; CNCL exclusion is instead locked through `tdw/--effort-counted-p',
...
;;;; ——— tdw/get-subtree-effort-minutes ———
...
")
    (goto-char (point-min))
    (assert-equal 105 (tdw/get-subtree-effort-minutes))))
...
(deftest subtree-effort/parent-with-no-effort-and-no-children-is-zero ()
  ...
  (with-temp-buffer
    ...
    (insert "* TODO Lonely\n")
    (goto-char (point-min))
    (assert-equal 0 (tdw/get-subtree-effort-minutes))))
...
")
    (goto-char (point-min))
    (assert-equal 45 (tdw/get-subtree-effort-minutes))))
...
")
    (goto-char (point-min))
    (assert-equal 30 (tdw/get-subtree-effort-minutes))))
...
")
    (goto-char (point-min))
    (assert-equal 60 (tdw/get-subtree-effort-minutes))))
...
")
    ...
    (assert-true (tdw/--effort-ancestor-counted-p))))
...
")
    (goto-char (point-max))
    (org-back-to-heading t)
    (assert-nil (tdw/--effort-ancestor-counted-p))))
